      ...
      data.data.forEach((report, index) => {
        $("#report_list_body").append(
          $("<tr>")
            ...
            .append($("<td>").append(report.submitter))
            .append($("<td>").append(report.description))
            .append($("<td>").append(formatTime(report.createdTime)))
            .append($("<td>").append(report.pdfReportStatus))
            .append($("<td>").append(report.excelReportStatus))
            .append(
              "<td>" +
                actionLinks(
                  report.pdfReportStatus,
                  report.excelReportStatus,
                  report.id
                ) +
                "</td>"
            )
...
function formatTime(time) {
  ...
  return (
    singleDigit(d.getMonth() + 1) +
    "/" +
    singleDigit(d.getDate()) +
    " " +
    singleDigit(d.getHours()) +
    ":" +
    singleDigit(d.getMinutes())
  );
}
...
function actionLinks(ps, es, id) {
  return (
    (ps === "COMPLETED"
      ? "<a onclick='downloadPDF(\"" + id + "\")' href='#'>Download PDF</a>"
      : "") +
    (es === "COMPLETED"
      ? "<a onclick='downloadExcel(\"" +
        id +
        "\")' style='margin-left: 1em' href='#'>Download Excel</a>"
      : "") +
    "<a onclick='showDelete(\"" +
    id +
    "\")' style='margin-left: 1em' href='#'>Delete</a>"
  );
}
